...
export async function validateAndGetFinalArtifact(
  ...
  // Dynamic minimum-size guard.
  // Goal: reject empty files, corrupt partials, and bot-detection HTML pages
  // (typically 1–10 KB) while always accepting legitimate media — even very
  // short clips (e.g. 4-second Pinterest HLS video ≈ 23 KB).
  //
  // Strategy:
  //   1. Run a quick FFprobe to get the real duration of the file.
  //   2. Minimum = max(5 KB,  duration_seconds × 1 KB)
  //      → A 4-second clip allows ≥4 KB  (23 KB passes ✅)
  //      → An empty / HTML-only file (0–8 KB) still fails ✅
  //   3. Hard-floor of 5 KB catches truly empty / zero-byte outputs.
  const ABSOLUTE_MIN_BYTES = 5 * 1024; // 5 KB — rejects zero-byte & HTML error pages
  let dynamicMinBytes = ABSOLUTE_MIN_BYTES;
  try {
    const quickProbe = await probeMediaFile(targetPath);
    if (quickProbe.duration && quickProbe.duration > 0) {
      // 1 KB per second of media — very conservative; accepts even low-bitrate clips
      dynamicMinBytes = Math.max(ABSOLUTE_MIN_BYTES, Math.ceil(quickProbe.duration) * 1024);
    }
  } catch {
    // If FFprobe fails to read the file at all, fall back to the 5 KB floor.
  }

  if (rawStats.size < dynamicMinBytes) {
    try { fs.unlinkSync(targetPath); } catch {}
    throw new Error(
      `Downloaded file failed validation: size is only ${(rawStats.size / 1024).toFixed(1)} KB ` +
      `(minimum expected is ${(dynamicMinBytes / 1024).toFixed(0)} KB for this media). ` +
      `The download may have been blocked or returned an error page instead of real media.`
    );
  }
...
export function validateDownloadedFile(filePath: string, type: 'video' | 'audio'): void {
  ...
  // Use a very low 5 KB floor — short clips (a few seconds) can legitimately be this small.
  // The goal is only to catch empty files and HTML bot-detection pages (usually < 5 KB).
  const minBytes = 5 * 1024;
  if (stats.size < minBytes) {
    try { fs.unlinkSync(filePath); } catch {}
    throw new Error(
      `Downloaded file failed validation: size is only ${(stats.size / 1024).toFixed(1)} KB ` +
      `(minimum expected is ${minBytes / 1024} KB). File appears empty or corrupted.`
    );
  }
}
