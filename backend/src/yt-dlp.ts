...
export async function getYtDlpMetadata(url: string) {
  ...
  for (const strategy of strategies) {
    try {
      ...
      const isUnsupported = errText.includes('Unsupported URL') || errText.includes('not a valid URL') || errText.includes('is not a valid URL');
      ...
      lastError = err;
      // On 429 or unsupported URL, no point trying more browsers — just bail
      if (is429 || isUnsupported) break;
