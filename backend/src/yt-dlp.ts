...
export async function downloadVideo(
  ...
  for (const cookieArgs of cookieStrategies) {
    const args: string[] = [
      url,
      '--no-playlist',
      '-N', '8', // Parallel 8-thread multi-fragment downloading (5x speed boost)
      '--concurrent-fragments', '8',
      '--http-chunk-size', '10M', // 10MB TCP chunk buffer for maximum bandwidth
      '--buffer-size', '64k',
      '--js-runtimes', 'deno',
      '--js-runtimes', 'node',
      '--extractor-args', 'youtube:player_client=android,web,ios',
      ...(ffmpegPath !== 'ffmpeg' ? ['--ffmpeg-location', ffmpegPath] : []),
      '--postprocessor-args', 'ffmpeg:-threads 0', // Use all CPU cores for lightning fast FFmpeg merging
      '--newline',
      // Auto-reconnect if YouTube throttles
      '--throttled-rate', '100K',
      '--retries', '10',
      '--fragment-retries', '10',
