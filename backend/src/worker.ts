...
http.createServer((req, res) => {
  try {
    ...
    if (url.pathname.startsWith('/files/')) {
      let filename = '';
      try {
        filename = decodeURIComponent(url.pathname.replace('/files/', ''));
      } catch (e) {
...
const worker = new Worker('downloads', async (job: Job) => {
  ...
  console.log(`[Worker] File URL check — WORKER_EXTERNAL_URL: ${process.env.WORKER_EXTERNAL_URL || 'unset'}, RENDER_EXTERNAL_URL: ${process.env.RENDER_EXTERNAL_URL || 'unset'}, resolved base: ${workerBaseUrl || 'EMPTY — will fall back to relative path, which is wrong'}`);
