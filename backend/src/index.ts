...
import { getYtDlpMetadata, downloadVideo, findActualOutputFile, getYtDlpPath, getFfmpegPath } from './yt-dlp';
...
app.get('/api/files/:filename', async (req, res) => {
  ...
  if (isRedis && workerBase) {
    ...
    try {
      ...
      res.set('Content-Disposition', inline ? 'inline' : `attachment; filename="${safeFilename}"; filename*=UTF-8''${encodeURIComponent(filename)}`);
