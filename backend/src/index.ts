...
import { getYtDlpMetadata, downloadVideo, findActualOutputFile, validateDownloadedFile, validateAndGetOutputFile, getYtDlpPath, getFfmpegPath } from './yt-dlp';
...
app.use(cors({
  ...
  exposedHeaders: ['Content-Range', 'Accept-Ranges', 'Content-Length', 'Content-Disposition', 'Content-Type'],
  credentials: false,
...
let cachedCookieStatus: { valid: boolean; message: string; raw?: string; timestamp: number } | null = null;
const COOKIE_CACHE_TTL = 5 * 60 * 1000; // 5 minutes cache

app.get('/api/cookie-status', async (req, res) => {
  const now = Date.now();
  if (cachedCookieStatus && (now - cachedCookieStatus.timestamp < COOKIE_CACHE_TTL)) {
    return res.json(cachedCookieStatus);
  }
  ...
  try {
    ...
    cachedCookieStatus = { valid: true, message: 'YouTube cookies are working.', timestamp: now };
    res.json(cachedCookieStatus);
  } catch (error: any) {
...
app.post('/api/resolve', async (req, res) => {
  ...
  try {
    ...
    const msg = error?.message || 'Failed to resolve metadata';
...
app.post('/api/download', downloadLimiter, async (req, res) => {
  ...
  if (isRedis) {
    ...
    (async () => {
      ...
      try {
        await downloadVideo(url, formatId, type, outputPath, (pct, stage) => {
          job.progress = pct;
          console.log(`[Download API] Job ${jobId} progress: ${pct}% (stage: ${stage || 'downloading'})`);
        });

        // Validate final output file, run FFprobe verification, and establish final artifact
        ...
        job.returnvalue = {
          ...
          downloadUrl: `/api/files/${encodeURIComponent(jobId)}/${encodeURIComponent(validated.filename)}`,
        ...
        console.log(`[Download Complete] Job: ${jobId}, File: ${validated.filename}, Final Bytes: ${validated.finalSizeBytes}, Exact Size: ${validated.finalSize}`);
      } catch (err: any) {
        job.status = 'failed';
...
// 4. Serve downloaded files (job-scoped: /api/files/:jobId/:filename)
// In Redis/worker mode the file lives on the worker container's disk, not here.
// We proxy it internally preserving Content-Length, Range, and Content-Disposition.
app.get('/api/files/:jobId/:filename', async (req, res) => {
  ...
  if (isRedis && workerBase) {
    const workerFileUrl = `${workerBase}/files/${encodeURIComponent(jobId)}/${encodeURIComponent(filename)}${inline ? '?inline=true' : ''}`;
    try {
      ...
      const upstream = await fetch(workerFileUrl, { headers });
      if (!upstream.ok && upstream.status !== 206) {
        console.error(`[API] Worker returned ${upstream.status} for file: ${jobId}/${filename}`);
      ...
      const disposition = upstream.headers.get('content-disposition') || (inline ? 'inline' : `attachment; filename="${safeFilename}"; filename*=UTF-8''${encodeURIComponent(filename)}`);
  ...
  // --- Local/in-memory mode: serve from this container's disk inside isolated job directory ---
  const TEMP_DIR = path.join(process.cwd(), 'temp');
  const jobDir = path.join(TEMP_DIR, jobId);
  
  // Prefer final artifact subfolder: temp/<jobId>/final/<filename>
  let filePath = path.join(jobDir, 'final', filename);
  if (!fs.existsSync(filePath)) {
    filePath = path.join(jobDir, filename);
  }

  // Path traversal guard: must stay strictly inside jobDir
  if (!filePath.startsWith(jobDir) || !fs.existsSync(filePath)) {
...
// Backward compatibility route for flat /api/files/:filename
app.get('/api/files/:filename', async (req, res) => {
  ...
  if (filePath.startsWith(TEMP_DIR) && fs.existsSync(filePath) && !fs.statSync(filePath).isDirectory()) {
    const stats = fs.statSync(filePath);
    const totalSize = stats.size;
    const isMp3 = filename.toLowerCase().endsWith('.mp3');
    const contentType = isMp3 ? 'audio/mpeg' : 'video/mp4';
...
app.post('/api/upload-cookies', async (req, res) => {
  ...
  try {
    ...
    // If running in worker mode, forward the cookies to the worker
    const workerBase = (process.env.WORKER_BASE_URL || '').replace(/\/$/, '');
    if (isRedis && workerBase) {
      const workerUrl = `${workerBase}/upload-cookies`;
      console.log(`[Cookies] Forwarding cookies to worker: ${workerUrl}`);
      try {
        ...
        if (response.ok) {
          console.log('[Cookies] Successfully synchronized cookies with the worker container');
        } else {
          console.warn('[Cookies] Failed to synchronize cookies with the worker container:', response.status);
        }
      } catch (err: any) {
        console.error('[Cookies] Error forwarding cookies to worker:', err.message);
      }
    }
...
app.get('/api/preview', (req, res) => {
  ...
  const isTikTok = /tiktok\.com/i.test(url as string);
...
// 7b. GET /api/thumbnail — proxy thumbnail images to avoid CORS issues
app.get('/api/thumbnail', async (req, res) => {
  const { url } = req.query;
  if (!url) return res.status(400).send('URL is required');
  try {
