...
export default function Home() {
  ...
  const handleResolve = async (customUrl?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        // If it's a rate-limiting or bot check error, reveal the cookie upload panel
        const errMessage = data.error || 'Failed to resolve URL'
        if (errMessage.includes("429") || errMessage.includes("rate-limiting") || errMessage.includes("Sign in") || errMessage.includes("bot")) {
                  ...
                  <div className="flex flex-col gap-3 pt-2">
                    ...
                    <div className="flex gap-3 justify-center mt-2">
                      ...
                      {downloadingJob.status === 'completed' && downloadingJob.result && (
                        ...
                        >
                          <a
                            href={`${API_URL}${downloadingJob.result.downloadUrl}?inline=true`}
                            target="_blank"
                            rel="noopener noreferrer"
                          >
      ...
      {metadata && !downloadingJob && (
        <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-brand-surface/90 backdrop-blur-xl border-t border-brand-border p-4 z-40 flex items-center justify-between animate-in slide-in-from-bottom-full duration-300">
          <div className="flex items-center gap-2.5 min-w-0 pr-2">
            ...
            <img src={metadata.thumbnail} className="w-10 h-10 object-cover rounded-lg border border-white/10 shrink-0" alt="" referrerPolicy="no-referrer" />
