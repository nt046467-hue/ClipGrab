...
import { Terminal, Send, X, Bot, User, CornerDownLeft, Loader2 } from "lucide-react"
...
export function ChatAssistant({ hasStickyBar = false }: ChatAssistantProps) {
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      // 1. Try local Next.js /api/chat route
      ...
      // 2. Fallback to active backend API URL if needed
      if (!response || !response.ok) {
        const storedUrl = typeof window !== 'undefined' ? (localStorage.getItem('clipgrab_custom_api_url') || '') : ''
        if (storedUrl) {
          response = await fetch(`${storedUrl}/api/chat`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ message: textToSend.trim() })
          }).catch(() => null)
      ...
      setMessages((prev) => [
        ...
        {
          sender: "engine",
          text: data.reply || "Support engine active. Feel free to ask any media download question.",
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ])
    } catch (err: any) {
      setMessages((prev) => [
        ...
        {
          sender: "engine",
          text: "Support Engine online! You can download 4K video, extract MP3s, or switch to 1-Click Colab for 100MB/s speeds.",
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ])
    } finally {
  ...
  // Prevent scroll on mobile when full-screen drawer is open
  useEffect(() => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 sm:inset-auto sm:bottom-24 sm:right-6 z-[60] w-full h-full sm:w-[400px] sm:h-[520px] animate-in slide-in-from-bottom-6 duration-300 flex flex-col justify-end sm:block">
          ...
          <Card className="relative w-full h-[90vh] sm:h-full flex flex-col bg-brand-surface border-brand-border sm:border shadow-2xl rounded-t-3xl sm:rounded-2xl overflow-hidden z-10">
            {/* Header */}
