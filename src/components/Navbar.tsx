    ...
    const checkHealth = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/health`, {
          cache: 'no-store',
          signal: AbortSignal.timeout(35000), // 35s timeout to allow Render free-tier cold starts
        })
        ...
        // Next check in 30 seconds when healthy
        timer = setTimeout(checkHealth, 30000)
      } catch (err) {
        setEngineStatus('down')
        // Retry faster (in 6 seconds) when down so we catch Render waking up ASAP
        timer = setTimeout(checkHealth, 6000)
      }
    }
