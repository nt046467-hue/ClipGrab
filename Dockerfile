...
# Install yt-dlp with optional dependencies (including curl-cffi for TikTok/Cloudflare browser impersonation) via Python venv
RUN python3 -m venv /opt/yt-dlp-venv \
    && /opt/yt-dlp-venv/bin/pip install --no-cache-dir --upgrade pip \
    && /opt/yt-dlp-venv/bin/pip install --no-cache-dir "yt-dlp[default,curl-cffi]" curl_cffi \
...
# Start the API server by default (the background worker overrides this command)
